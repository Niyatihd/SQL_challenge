...
####TEST
SELECT * FROM actor WHERE Actor_Name = 'ED CHASE';
...
FROM actor
where first_name = 'HARPO' or last_name = 'WILLIAMS';
--   	
...
SELECT actor_id, first_name, last_name
FROM actor
where first_name = 'GROUCHO' OR last_name = 'WILLIAMS';
...
address_id SMALLINT(5) UNSIGNED NOT NULL PRIMARY KEY,
address VARCHAR(50),
address2 VARCHAR(50),
district VARCHAR(20),
...
SELECT f.title, COUNT(*) RentalCount
FROM rental r
...
SELECT i.store_id, SUM(p.amount) Total_Rent_Collected
FROM payment p
...
SELECT c.name Genre, SUM(p.amount) Total_Revenue
FROM category c
